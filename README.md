...
- [**Video controls in settings**](https://github.com/bh679/chess-client/wiki/Feature:-Remove-Video-Overlay) — removed floating video overlay and camera preview modal; video controls (mic, camera, end-call) now live in a "Video Call" section in the settings panel, visible only during active calls
...
- [**Public lobby list below board**](https://github.com/bh679/chess-client/wiki/Feature:-Public-Lobby-List) — open public games appear in a "Players ready and waiting for a game" panel directly below the board, auto-refreshing every 5 seconds; each listing shows the host name, time control, variant, and camera mode as a large rounded card with a green outlined Join button; players can join with one click without opening the New Game wizard
...
- [**Tile Cam mode**](https://github.com/bh679/chess-client/wiki/Feature:-Tile-Cam) — each of the 64 board tiles shows its own complete camera feed; white player's feed fills all light squares, black player's feed fills all dark squares (like 32 small monitors per colour); face tracking and turn tints apply as with other modes
